...
string getboundary(const string& boundaryline)
{
	string boundarysuffix = boundaryline.substr(boundaryline.find("boundary=")+9);
	if (boundarysuffix[0]=='\"')
	{
...
int32_t main()
{
	...
	string boundary = getboundary(getboundaryline());
	assert(boundary.size());
	string startbound = "--" + boundary;
	string endbound = startbound + "--";
