...
if correct == "ej magisk":
    ...
    if outputdata == "ej magisk":
        accept()
    else:
        die("failed to detect that there is no solution")
...
try:
    ...
    if len(outputdata) != R:
        die("incorrect number of rows")
    for x in outputdata:
        if len(x) != C:
            die("incorrect number of tokens in a row")
except:
    die("error")
...
try:
    foutputdata = [[tofrac(x) for x in y] for y in outputdata]
except:
    die("failed to parse some token as a fraction")
