...
For each rabbit Mårten makes appear or disappear, he must use 1 magicks.
In the beginning of the show, Mårten has $K$ magicks. When he has run out
of magicks, he can no longer perform a trick.

The scoring of the competition is easy. In each round, let $x$ the
...
You should implement the function \texttt{magic\_score(N, K, L, R)} and \texttt{play(X)}.
\begin{itemize}
...
For each subtask, if the return value of \texttt{magic\_score(N, K, L, R)} is correct, but your calls to \texttt{play(X)}
